...
export const deleteUser = async (req, res) => {
    try {
        ...
        await Group.updateMany({ users: req.params.userId }, { $pull: { users: req.params.userId } });
...
export const addPost = async (req, res) => {
    try {
        const user = await User.findByIdAndUpdate(req.params.userId, { $addToSet: { posts: req.body } }, { new: true, runValidators: true });
        if (!user) {
...
export const removePost = async (req, res) => {
    try {
        const user = await User.findByIdAndUpdate(req.params.userId, { $pull: { posts: { postId: req.params.postId } } }, { new: true, runValidators: true });
        if (!user) {
            return res.status(404).json({ message: "No user found with that ID :(" });
        }
        return res.json(user);
